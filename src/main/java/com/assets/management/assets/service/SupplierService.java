...
//	QrProxy qrProxy;
	...
	public URI createVendor(@Valid Supplier vendor, @Context UriInfo uriInfo) {
		...
		return uriInfo.getAbsolutePathBuilder().path(Long.toString(vendor.id)).build();
	}
...
// to be deleted
	public URI addAsset(@Valid Item asset, @NotNull Long vendorId, @Context UriInfo uriInfo) {
...
//        asset.qrString = qrCodeString.formatCodeImgToStr(asset);
		...
		return uriInfo.getAbsolutePathBuilder().path(Long.toString(asset.id)).build();
	}

	@Transactional(Transactional.TxType.SUPPORTS)
	public List<Item> getAllAssets(Long vendorId, Integer index, Integer size) { // "select a from Asset a where
																					// a.vendor.id = ?1",
