...
@Tag(name = "Computer Endpoint", description = "Inventory of all computers")
...
//    LinkHeaderPagination headerPagination;
    ...
    @GET
    @Operation(summary = "Retrieves all computers")
    @APIResponses({
            @APIResponse(
                    responseCode = "200",
                    content = @Content(
                            mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = ComputerEntity.class, type = SchemaType.ARRAY)),
                    description = "Lists all the computers"),
            @APIResponse(responseCode = "204", description = "No computer to display"),
    })
    public Response allComputers(
            ...
            @Parameter(description = "Search date", required = false) @QueryParam("date") String localDate,
...
//        PanacheQuery<AssetEntity> query = Computer.getAll(search, date, column, direction);
...
//                .build();
        ...
        PanacheQuery<AssetEntity> computerAssets = assetRepository.getAll(search, LocalDate.parse(localDate));
        Page currentPage = Page.of(pageIndex, pageSize);
        computerAssets.page(currentPage);

        List<ComputerEntity> computers = computerAssets.list();
        return Response.ok(computers)
